...
def registrar_cliente(request):
    ...
    if request.method == 'POST' and 'buscar_cliente' in request.POST:
        ...
        if cliente_form.is_valid():
            ...
            if any(request.POST.get(field) for field in medida_form.fields):
            ...
            if any(request.POST.get(field) for field in tipo_lunas_form.fields):
                if tipo_lunas_form.is_valid():
                    luna = tipo_lunas_form.save(commit=False)
                    luna.cliente = cliente
                    luna.save()
                    print("✅ Tipo de lunas registrada")
    ...
    medidas =MedidaVista.objects.all()
